...
def perimeter_detection(weights_path, video_path, result_path, threshold, perimeter_a, perimeter_b):
    ...
    while (cap.isOpened()):
        ...
        resize_image = cv2.resize(frame, (img_height, img_width))
    ...
    result_video = cv2.VideoWriter('result.mp4', fourcc, 20.0, (original_images[0].shape[0], original_images[0].shape[1]))
    ...
    for k in range(len(y_pred_thresh)):
        ...
        plt.savefig(result_path + '/detection_' + str(k), format='jpg')
        result_image = cv2.imread(result_path + '/detection_' + str(k))
